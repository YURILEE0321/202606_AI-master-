...
_SYSTEM_PROMPT_PATH = (
    Path(__file__).resolve().parent.parent.parent / "개발환경" / "AI Wiki Assistant Agent - System Prompt.md"
)
...
사용자의 자연어 질문을 분석하여 의도(intent)와 검색에 사용할 핵심 키워드(keywords)를 추출한다.

intent는 다음 중 하나로 분류한다: "기능문의", "용어", "메뉴/업무절차", "데이터유입문제", "기타".
keywords는 질문에 등장하는 메뉴명, 기능명, 용어, 설정명 등 검색에 유용한 명사구 위주로 3~6개 추출한다."""


def build_answer_generator_prompt(question: str, context: str) -> str:
...
def _attempt_context(keywords: List[str], context: str) -> str:
...
def build_query_rewriting_prompt(original_question: str, keywords: List[str], context: str) -> str:
...
{_attempt_context(keywords, context)}

원본 질문의 의도는 유지하되, 모호한 표현을 구체화하고 플랫폼 용어(위 힌트 참고)로 바꾸어 검색에 더 적합한 한 문장으로 다시 작성하라."""


def build_multi_query_prompt(original_question: str, keywords: List[str], context: str) -> str:
    return f"""당신은 AI Wiki Assistant의 Query Rewriter이다(2차 재시도, 기법: Multi Query Retrieval).
...
def build_query_expansion_prompt(original_question: str, keywords: List[str], context: str) -> str:
    return f"""당신은 AI Wiki Assistant의 Query Rewriter이다(3차 재시도, 기법: Query Expansion).
